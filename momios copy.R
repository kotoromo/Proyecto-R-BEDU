...
dt1119<-do.call(rbind,lista)                       #Usando do.call, combina las listas en un data frame

###############################Formato de Columnas##############################

dt1119 <- dplyr::rename(dt1119, Max.2.5.O = BbMx.2.5,    #Usando rename del paquete dplyr, se cambian los nombres de las columnas para el data frame 2014-2019
                 Avg.2.5.O = BbAv.2.5, 
...
dt1119 <- select(dt1119, colnames(e1920))                #Se ponen las columnas en el mismo orden para los dos data sets

################################Unificación#####################################

dt1119 <- rbind(dt1119, e1920)                     #Usando rbind, se combinan por filas los dataframes para tener los datos del 2014-2020

###############################Formato de Columnas##############################

dt1119 <- dplyr::rename(dt1119, date = Date, home.team = HomeTeam,   #Renombra las columnas del dataframe
                        home.score = FTHG, away.team = AwayTeam, 
                        away.score = FTAG)

data <- select(dt1119, date, home.team, home.score,                  #Selecciona las columnas que se usarán en el órden que se desean
...
bar<-subset(visitante, away.team =="Barcelona")
ggplot(bar,aes(away.score))+
  geom_histogram(binwidth = 1,col="black",fill="light blue")+
  ggtitle("Barcelona")+
  xlab("Goles como equipo visitante")+
  ylab("Frecuencia")+
  theme_light()
...
set.seed(23)
bar_40<- sample(bar$away.score,40)                            #Selecciona una muestra aleatoria de tamaño 40
bar_40<- as.data.frame(bar_40)
bar_40 <- dplyr::rename(bar_40, away.score=bar_40)

mean<- mean(bar_40$away.score)
sd<- sd(bar_40$away.score)
n<- length(bar_40$away.score)
...
texto<- if(z0< z.05){
  print("Se falla en rechazar la hipótesis nula")
}else(
...
text(2, 0.25, labels = texto, col = "red")
